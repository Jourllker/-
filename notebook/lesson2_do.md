# 轻松玩转书生·浦语大模型趣味 Demo笔记
# ！！！注：由于教程是傻瓜式教程，非常详细，所以我的笔记完全从教程上进行一定的增删改。由于是实践操作，教程本身非常详细，所以基本内容同教程一样。
...
### **2.1 配置基础环境**
`创建开发机` >> 填写 `开发机名称` >> 选择镜像`Cuda11.7-conda`  >> 使用 `10% A100 * 1` >> 点击 `进入开发机` 。

**在 jupyter notebook 的`terminal` 中输入环境配置命令 (配置环境时间较长，需耐心等待)：**
...
通过左侧文件夹栏目，`双击`进入 `demo` 文件夹。双击打开 `/root/demo/download_mini.py` 文件，复制以下代码：
...
![alt text](images/lesson2/img-6.png)
...
![alt text](images/lesson2/img-8.png)

打开 PowerShell 后，先查询端口，再根据端口键入命令 （例如图中端口示例为 38374）：

![alt text](images/lesson2/img-A.png)
...
![alt text](images/lesson2/img-B.png)

最终保持在如下效果即可：

![alt text](images/lesson2/img-C.png)

打开 [http://127.0.0.1:6006](http://127.0.0.1:6006) 后，等待加载完成即可进行对话。

效果图如下：

![alt text](images/lesson2/img-D.png)
...
![alt text](images/lesson2/Lagent-1.png)
...
![alt text](images/lesson2/img-E.png)
...
![alt text](images/lesson2/img-F.png)
...
![alt text](images/lesson2/img-G.png)
...
![alt text](images/lesson2/img-H.png)
...
![alt text](images/lesson2/img-I.png)

## 5 **实战：实践部署 `浦语·灵笔2` 模型（开启 50% A100 权限后才可开启此章节）**

### 5.1 **初步介绍 `XComposer2` 相关知识**
`浦语·灵笔2` 是基于 `书生·浦语2` 大语言模型研发的突破性的图文多模态大模型，总结起来其具有：
- 自由指令输入的图文写作能力： `浦语·灵笔2` 可以理解自由形式的图文指令输入，包括大纲、文章细节要求、参考图片等。
- 准确的图文问题解答能力
- 杰出的综合能力： `浦语·灵笔2-7B` 基于 `书生·浦语2-7B` 模型，在13项多模态评测中大幅领先同量级多模态模型，在其中6项评测中超过 `GPT-4V` 和 `Gemini Pro`。

![alt text](images/lesson2/Benchmark_radar_CN.png)

### 5.2 **配置基础环境（开启 50% A100 权限后才可开启此章节）**

选用 `50% A100` 进行开发：

![alt text](images/lesson2/check-4.png)
...
![alt text](images/lesson2/img-9.png)
...
![alt text](images/lesson2/img-7.png)
...
![alt text](images/lesson2/check-3.png)
